...
```bash
# Get a token
TOKEN=$(curl -s -X POST 'http://localhost:8080/realms/demo/protocol/openid-connect/token' -H 'Content-Type: application/x-www-form-urlencoded' -d 'grant_type=password' -d 'client_id=flask-app' -d 'client_secret=dev-secret' -d 'username=clinician1' -d 'password=password' | python3 -c "import sys,json; print(json.load(sys.stdin)['access_token'])")
...
| SSE /api/streaming/heartbeat/till-denied | `enforce_till_denied` | None | Terminal denial |
| SSE /api/streaming/heartbeat/drop-while-denied | `enforce_drop_while_denied` | None | Silent drops during DENY |
| SSE /api/streaming/heartbeat/recoverable | `enforce_recoverable_if_denied` | None | In-band deny/recover signals |
