...
**How JWT flows through the system:** FastAPI validates the JWT via JWKS. The enforcement code passes the raw token to the PDP via the `secrets` field. The SAPL policy reads `<jwt.token>.payload.pilotId` to make the authorization decision.
