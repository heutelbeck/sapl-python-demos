...
views_services.py delegates to these methods.

Uses ``@service_pre_enforce`` / ``@service_post_enforce`` decorators from
sapl_django, which handle subscription building and PDP communication
...
from sapl_django.decorators import service_post_enforce, service_pre_enforce
